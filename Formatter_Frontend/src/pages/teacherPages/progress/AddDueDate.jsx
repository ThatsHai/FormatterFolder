...
const AddDueDate = ({
  ...
  const senData = async () => {
    ...
    try {
      const response = await api.put(`/milestones/setDueDate`, payload);
  ...
  const previousMilestone = milestones.find((m) => m.position === milestone.position-1);
  const nextMilestone = milestones.find((m) => m.position === milestone.position+1);
  const minDate =
  ...
  return (
    <div>
      ...
      {showConfirmPopup && (
        <ConfirmationPopup
          isOpen={true}
          text={"Bạn chắc chắn muốn đặt ngày hết hạn?"}
          onConfirm={() => {
            setShowConfirmPopup(false);
            senData();
          }}
      ...
      {displaySuccessPopup && (
        <SuccessPopup
          isOpen={true}
          successPopupText={"Đặt ngày hết hạn thành công!"}
